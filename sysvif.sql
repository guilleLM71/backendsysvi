...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 12-01-2023 a las 20:38:13
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_detalle_temp` (IN `codigo` INT, IN `cantidad` INT, IN `token_user` VARCHAR(200))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `del_detalle_temp` (IN `id_detalle` INT, IN `token` VARCHAR(200))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getdatosdash` ()  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_factura` (IN `nofactura` INT(10))  BEGIN
select 
f.nofactura,
f.fecha,
f.totalfactura,
...
where f.codcliente=c.idcliente
and f.nofactura=nofactura;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `procesar_venta` (IN `cod_usuario` INT, IN `cod_cliente` INT, IN `token` VARCHAR(200))  BEGIN
DECLARE factura INT;
...
WHILE a <= registros DO
    ...
    UPDATE producto SET existencia = nueva_existencia 
    WHERE codproducto = tmp_cod_producto;
...
UPDATE factura SET totalfactura = total 
WHERE nofactura = factura;
...
(1, 123, 'usuariox', 123123, 'La Paz', 1);
...
CREATE TABLE `detalle_temp` (
  ...
  `token_user` varchar(200) COLLATE utf8_spanish_ci NOT NULL,
  `codproducto` int(11) NOT NULL,
  `cantidad` int(11) NOT NULL,
  `precio_venta` decimal(10,2) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_spanish_ci;
...
INSERT INTO `empresa` (`id`, `dni`, `nombre`, `razon_social`, `telefono`, `email`, `direccion`) VALUES
(1, 123, 'SA', 'SA', 78889375, 'emp@email.com', 'Bolivia');
